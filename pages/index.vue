  ...
  methods: {
    ...
    async editItem(item, DialogOrder){ 
        ...
        this.$axios.post('https://app.noest-dz.com/api/public/get/tracking/info', {
            ...
            if(item.statut != 'completed' && data.data.activity && data.data.activity.length > 2 && data.data.activity.map(act => act.event).includes('Montant transmis au partenaire')){ 
                await this.changeStateOrder('completed')
            }else if(item.statut != 'rejected' && data.data.activity && data.data.activity.length > 2 && data.data.activity.map(act => act.event).includes(key => key == 'Suspendu') && data.data.activity.map(act => act.event).includes(key => key == 'Retour transmis au partenaire')){     
